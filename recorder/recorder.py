...
Dumb capture: connects to the ESP32's serial stream (three comma-separated
values per line -- ch1,ch2,ch3) and writes every sample straight to a CSV,
...
def record(port: str, baud: int, duration_s: float, csv_path: Path) -> None:
    ...
    while time.time() < deadline:
        ...
        if start_time is None:
            start_time = now
        rows.append((now - start_time, ch1, ch2, ch3))
        print(line)
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["elapsed_s", "ch1", "ch2", "ch3"])
...
DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "v9"


def main() -> None:
    parser = argparse.ArgumentParser(description="Record the three-channel EEG serial stream to a CSV for later analysis")
