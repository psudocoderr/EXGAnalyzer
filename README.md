...
| Path | Contents |
|------|----------|
| `firmware/BioSignal-Recorder-v9/` | Current sketch: three-channel ESP32 (GPIO33/35/34) at 250 Hz over serial, one `synapse.h` filter per channel. |
...
1. Flash `firmware/BioSignal-Recorder-v9/BioSignal-Recorder-v9.ino` to an ESP32
...
2. Install Python dependencies:
   ...
   ```
3. Record and analyse (sessions land in `data/v9/`):
   ```bash
   python recorder/recorder.py --port /dev/ttyUSB0 --duration 30
   python recorder/analyzer.py --csv data/v9/eeg_session_<timestamp>.csv
   ```
